...
def extract_features(sample: V6TrainingData) -> np.array:
    return np.array(sample.planes[0:NUM_PLANES], dtype=np.uint64)
        ...
        for piece in (
            ...
            for square in range(BOARD_SIZE):
                if int(plane) & (1 << square):
                    ...
                    corrected_square = square - (square % 8)
                    corrected_square += 7 - (square % 8)
                    board.set_piece_at(
                        corrected_square, chess.Piece(piece, color)
                    )
            plane_id += 1
...
def process_archive(archive_path: str, features_path, targets_path) -> int:
    ...
    print(f"num_samples after deduplication: {num_samples}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(features_path) or os.path.exists(targets_path):
        print(
            f"The data file for {input_filename} already exists, will not run again"
        )
        sys.exit()
