...
class DataLoader:
    ...
    def __init__(self, features_path, targets_path, batch_size):
        ...
        assert (
            self.num_train_samples + self.num_dev_samples + self.num_test_samples
            == self.num_samples
        )
    ...
    def _iter(self, start_idx, end_idx, shuffle):
        ...
        for batch_idx in tqdm.tqdm(batch_indices):
            ...
            batch_end_idx = min(end_idx, start_idx + (batch_idx + 1) * self.batch_size)
...
if __name__ == "__main__":
    # loader = DataLoader(
    # "./sf-nnue/features_test.data", "./sf-nnue/targets_test.data", BATCH_SIZE
    # )

    loader = DataLoader(
        "./data/processed/features.data", "./data/processed/targets.data", batch_size=10
    )
    ...
    """
  for features, targets in tqdm.tqdm(loader.train_ds()):
